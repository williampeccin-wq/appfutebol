...
// A janela do fixture começa em JULHO de propósito: nesta branch (instalação do
// Harmonia) as 5 rodadas importadas do Rei da Quadra são deste clube e entram em
// qualquer janela que cubra maio. Começar depois delas mantém o teste falando da
// janela da temporada, e não do dataset importado.
const snapshot = () => ({
  players: [
...
// 2) Apagar as datas NÃO deixa a temporada sem janela nesta branch: o estado
//    cai no fallback de ACTIVE_CHAMPIONSHIP (01/05–31/08), que é a temporada
//    desta instalação. Fica registrado porque é a diferença de comportamento
//    para o app genérico, onde clube sem datas soma tudo.
{
  const estado = snapshot();
  estado.championship.active.start_date = '';
  estado.championship.active.end_date = '';
  assert.deepEqual(getSeasonWindow(estado), { start: '2026-05-01', end: '2026-08-31' },
    'sem datas explícitas vale a temporada da constante, não "sem janela"');
...
// 5) Encerrar congela pontos E notas, migra o que estava fora e zera o resto.
{
  ...
  // 6) Temporada encerrada é FATO: mexer nos lançamentos depois não a reescreve.
  estado.championship.active.results = [];
  assert.equal(getFrozenSeasons(estado)[0].rows.find((r) => r.player_id === 'p_zeca').points, 3,
    'o congelado não pode ser recalculado a partir dos resultados de agora');
  ...
  estado.championship.active.results = [rodada('r_setembro', '2026-09-02', JOGO_SETEMBRO, { p_zeca: 'win', p_bibi: 'loss' })];
...
// 10) A sugestão da próxima temporada herda a duração da que está sendo encerrada.
{
  ...
  assert.equal(sugestao.end_date, '2026-10-31', '2 meses, igual à janela que fechou');
...
//     só se consertava congelando outra temporada pela metade.
{
  ...
  const impacto = seasonWindowChangeImpact(estado, { start: '2026-07-01', end: '2026-09-30' });
  assert.deepEqual(impacto.entering.map((r) => r.id), ['r_setembro']);
  assert.deepEqual(impacto.leaving, []);

  assert.equal(updateSeason(estado, { name: 'Inverno 26', start_date: '2026-07-01', end_date: '2026-09-30' }).ok, true);
  ...
  const volta = seasonWindowChangeImpact(estado, { start: '2026-07-01', end: '2026-08-31' });
  assert.deepEqual(volta.leaving.map((r) => r.id), ['r_setembro']);
  updateSeason(estado, { name: 'Inverno 26', start_date: '2026-07-01', end_date: '2026-08-31' });
