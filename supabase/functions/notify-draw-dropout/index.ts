...
//   1) o chamador tem sessão válida;
...
//   5) envia para os administradores (menos quem saiu, se for admin).
...
Deno.serve(async (req) => {
  ...
  // 4) Destinatários: administradores, menos quem saiu.
  const { data: admins } = await admin
    .from("players").select("id").eq("is_admin", true);
