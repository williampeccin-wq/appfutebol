-- Protege data.pending nos triggers de guarda do players.
--
-- Auto-cadastro entra com pending=true (aguardando aprovação do admin). Sem esta
-- guarda, um jogador pendente poderia se AUTO-APROVAR gravando pending=false via
-- update do próprio perfil (os triggers só barravam mens_ok/role/plays_football/
-- in_carne_group/carne_group/is_admin). Aqui adicionamos pending à lista.
--
-- Admin e service_role seguem liberados (atalho no topo de cada função) — é assim
-- que o admin APROVA (seta pending=false) e o register-player cria pendente.
-- Corpo idêntico ao de 20260617000000_allow_service_role_mens_ok, + guarda de pending.
...
begin
  ...
  if coalesce(NEW.data->>'pending', '') is distinct from coalesce(OLD.data->>'pending', '') then
    raise exception 'pending_is_admin_only';
  end if;
...
begin
  ...
  if coalesce(old.data->>'pending', '') is distinct from coalesce(new.data->>'pending', '') then
    raise exception 'Only admins can change pending';
  end if;
