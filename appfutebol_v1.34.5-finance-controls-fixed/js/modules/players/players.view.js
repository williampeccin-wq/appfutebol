...
function renderFinanceControls(player, currentPlayer) {
  ...
  if (isFinancePending(player)) {
    return `<button class="btn btn-secondary" data-action="mark-paid" data-id="${player.id}">Marcar pago</button>`;
  }

  return `<button class="btn btn-primary" data-action="mark-debt" data-id="${player.id}">Marcar inadimplente</button>`;
}
