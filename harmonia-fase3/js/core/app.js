      ...
      <section class="card">
        ...
        <div class="session-card compact">
          ...
          <div class="chip-row">
            ...
            <span class="tag ${carneStatus ? 'is-neutral' : 'is-neutral'}">${carneStatus ? 'Grupo da carne ativo' : 'Sem grupo da carne'}</span>
          </div>
        </div>
      </section>
...
function renderConfig(snapshot, currentPlayer) {
  ...
  return `
    <section class="section-stack">
      <section class="card">
        <div class="card-title">Fase 3 concluída</div>
        <div class="info-block">
          <div class="info-line">• Sessão integrada à UI principal</div>
          <div class="info-line">• Distinção entre jogador, carne e admin</div>
          <div class="info-line">• Permissão base aplicada na navegação</div>
          <div class="info-line">• Tela de jogadores separada por perfis</div>
        </div>
      </section>
      ...
      <section class="card">
        ...
        <p class="footer-note">
          Players: ${snapshot.players.length} · Jogadores: ${snapshot.players.filter((player) => player.role !== 'carne').length} · Carne: ${snapshot.players.filter((player) => player.role === 'carne').length}
        </p>
      </section>
    </section>
  `;
