...
function rotuloTime(ctx, texto, y, cor, uniforme) {
  ...
  // empilhados na vertical). A camisa é o que identifica o time, então recorto
  // só o topo do mockup. Se a imagem já for ~quadrada (só a camisa), uso inteira.
  if (uniforme) {
    ...
    // Recorte arredondado para a camisa virar uma plaquinha limpa.
    ctx.save();
    traca();
    ctx.clip();
...
export async function gerarImagemEscalacao(snapshot, { titulo = 'ESCALAÇÃO', escudoUrl = ESCUDO_PADRAO, marcaRodape = 'convocados.app.br' } = {}) {
  ...
  // Opcional: time sem uniforme só não ganha selo.
