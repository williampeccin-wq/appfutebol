...
create index if not exists idx_players_is_admin on public.players (is_admin);

update public.players
set
  auth_user_id = coalesce(auth_user_id, data->>'auth_user_id'),
  is_admin = coalesce(is_admin, false) or lower(coalesce(data->>'is_admin', 'false')) in ('true', '1', 'yes', 'sim')
where auth_user_id is distinct from coalesce(auth_user_id, data->>'auth_user_id')
   or is_admin is distinct from (coalesce(is_admin, false) or lower(coalesce(data->>'is_admin', 'false')) in ('true', '1', 'yes', 'sim'));

update public.players
set data = jsonb_set(
  jsonb_set(
    jsonb_set(coalesce(data, '{}'::jsonb), '{id}', to_jsonb(id), true),
    '{auth_user_id}', coalesce(to_jsonb(auth_user_id), 'null'::jsonb), true
  ),
  '{is_admin}', to_jsonb(is_admin), true
);
...
drop trigger if exists trg_harmonia_guard_player_insert on public.players;
create trigger trg_harmonia_guard_player_insert
before insert on public.players
for each row execute function public.harmonia_guard_player_insert();

drop trigger if exists trg_harmonia_guard_player_update on public.players;
...
drop policy if exists "harmonia_app_state_read_authenticated" on public.app_state;
drop policy if exists "harmonia_app_state_write_admin" on public.app_state;
drop policy if exists "harmonia_players_read_authenticated" on public.players;
drop policy if exists "harmonia_players_insert_admin_or_self" on public.players;
drop policy if exists "harmonia_players_update_admin_or_self" on public.players;
drop policy if exists "harmonia_players_delete_admin" on public.players;
drop policy if exists "harmonia_game_read_authenticated" on public.game_state;
drop policy if exists "harmonia_game_write_admin" on public.game_state;
drop policy if exists "harmonia_confirmations_read_authenticated" on public.confirmations;
drop policy if exists "harmonia_confirmations_insert_admin_or_self" on public.confirmations;
drop policy if exists "harmonia_confirmations_update_admin_or_self" on public.confirmations;
drop policy if exists "harmonia_confirmations_delete_admin_or_self" on public.confirmations;
drop policy if exists "harmonia_meta_read_authenticated" on public.app_meta;
drop policy if exists "harmonia_meta_write_admin" on public.app_meta;
