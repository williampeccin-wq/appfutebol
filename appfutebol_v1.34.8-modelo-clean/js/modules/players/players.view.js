...
function renderFinanceControls(player, currentPlayer) {
  ...
  if (player.role === "carne") return "";
  if (player.role === "admin") return "";

  if (!player.mens_ok) {
    return `<button class="btn btn-small btn-success" data-action="mark-paid" data-id="${player.id}">Marcar pago</button>`;
  }
  return `<button class="btn btn-small btn-warn" data-action="mark-debt" data-id="${player.id}">Marcar inadimplente</button>`;
}
import {
  formatPhone,
  getInitials,
