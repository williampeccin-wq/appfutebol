...
function getFinancePriority(player) {
  const isAdmin = player.is_admin;
  const isCarne = player.role === 'carne';

  if (!isAdmin && !isCarne && !player.mens_ok) return 0;
  if (!isAdmin && !isCarne && player.mens_ok) return 1;
  if (isAdmin) return 2;
  return 3;
}
...
const orderedPlayers = [...sourcePlayers].sort((a, b) => {
  const pA = getFinancePriority(a);
  const pB = getFinancePriority(b);
  ...
  const jogadores = orderedPlayers.filter((player) => player.role !== 'carne');
  const carneOnly = orderedPlayers.filter((player) => player.role === 'carne');
